...
#End Region
        ...
        Try
            ...
            If AnnID <> 0 Then
                ...
                If Not IsNothing(DR) Then
                    ...
                    '    ImgFile.ImageUrl = gsFilePath & "NEWS/" & DR("IMAGE_FILE")
                    'Else
    ...
    Private Sub LoadFileData()
        ...
        Try
            If AnnID <> 0 Then
                DT = GetDT(LoadNewsFile(AnnID))
                If Not IsNothing(DT) Then
                    ImgFile.Visible = False
                    For Each DR In DT.Rows
                        If Not IsNothing(DR) Then
                            'Img = New HtmlImage
                            'Img.ID = "IMG_" & DR("FILE_ID") & ""
                            'Img.Src = gsFilePath & "NEWS/" & DR("SYS_FILE")
                            ImgHtml += "<IMG id='" & "IMG_" & DR("FILE_ID") & "' alt='' src='" & gsFilePath & "NEWS/" & DR("SYS_FILE") & "'><br> "
                        End If
                    Next
                    If ImgHtml <> "" Then lblImage.Text = ImgHtml
                Else
                    ImgFile.Visible = False
                    ImgFile.ImageUrl = "../images/nopic.gif"
                End If
            End If
            lblImage.Visible = (ImgHtml <> "")
