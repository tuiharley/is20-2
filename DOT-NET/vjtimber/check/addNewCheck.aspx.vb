    ...
    Public Function showCheck(ByVal st As String) As String
        ...
        If myQ.Count > 0 Then
            For Each myCheck In myQ
                ...
                str = str & "<td width='17%'>" & myCheck.CHECK_DATE.ToString("dd/MM/yyyy") & "</td>"
                str = str & "<td width='13%' bgcolor='#99FFFF'>ผู้สั่งจ่าย</td>"

                myCust = CustomerDB.getA_customer(myCheck.CUST_ID)
                str = str & "<td width='23%' colspan='2'>" & TitleDB.getTitleName(myCust.CUST_TITLE) & " " & myCust.CUST_NAME & " " & myCust.CUST_SURNAME & "</td>"
                str = str & "<td width='10%' bgcolor='#99FFFF' >ธนาคาร</td>"
                str = str & "<td width='25%' colspan='2' align='center'>" & BankDB.getBankName(myCheck.BANK_CODE) & "</td>"
                str = str & "</tr>"
                str = str & "<tr height='30'>"
                str = str & "<td bgcolor='#99FFFF'>เลขที่</td>"
                str = str & "<td>" & myCheck.CK_NO & "</td>"
                str = str & "<td bgcolor='#99FFFF'>จำนวนเงิน</td>"
                str = str & "<td colspan='2' align='right'>" & FormatNumber(myCheck.AMOUNT, 2, , , TriState.True) & " </td>"
                str = str & "<td bgcolor='#99FFFF'>สถานะ</td>"
                str = str & "<td colspan='2' align='center'>" & StatusDB.getStatusName(myCheck.CK_STATUS) & "</td>"
                str = str & "</tr>"
                str = str & "<tr height='30'>"
                str = str & "<td bgcolor='#99FFFF'>หมายเหตุ</td>"
                str = str & "<td colspan='4'> " & myCheck.CK_DETAIL & " </td>"
                str = str & "<td colspan='3'>"
                str = str & " <input name='btnDel' type='button' class='btn' id='btnDel' value='ลบ' onClick=""delCheck('" & myCheck.CHECK_ID & "')""/>"
                str = str & " <input name='btnEdit' type='button' class='btn' id='btnEdit' value='แก้ไข' /></td>"
                str = str & "</tr>"
                str = str & "</table>"
                str = str & "<table>"
                str = str & "<tr height='1'>"
