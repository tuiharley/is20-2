Public Class dealer_business_web_contact
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        AjaxPro.Utility.RegisterTypeForAjax(GetType(dealer_business_web_contact))
    End Sub
    ...
    Public Function showTitle() As String
        Dim sql As String, ck As String
        Dim myQTitle As Queue = TitleDB.getTitle()
        Dim myTitle As title
        Dim i As Integer = 1

        sql = "คำนำหน้า "
        For Each myTitle In myQTitle
            If i = 1 Then
                ck = "checked"
            Else
                ck = ""
            End If
            sql = sql & "<input name='tit' id='tit' type='radio' value='" & myTitle.titlename & "' " & ck & "> " & myTitle.titlename
            i = i + 1
        Next
    ...
    <AjaxPro.AjaxMethod()> _
    Public Function sendEmail(ByVal title As String, ByVal name As String, ByVal request As String, ByVal phone As String, ByVal callback As String, ByVal email As String, ByVal more As String) As String
        Dim sql As String
        Dim myQuestion As EmailDB.mailRequest

        myQuestion.reqTo = Session("dealer_id")
        myQuestion.title = title
        myQuestion.name = name
        myQuestion.request = request
        myQuestion.phone = phone
        myQuestion.callback = callback
        myQuestion.email = email
        myQuestion.more = more

        EmailDB.sendMailContact(myQuestion)
        sql = "OK"
