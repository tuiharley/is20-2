...
#End Region
    ...
    Private Sub enginetype_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles enginetype.Init
        ...
        conn.Close()
    End Sub
    Private Sub Drive_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles drive.Init
        Dim conn As OleDbConnection
        Dim da As OleDbDataAdapter
        Dim ds As New DataSet
        Dim Sql As String
        ...
        DRow.Item("DRIVE") = 0
        DRow.Item("DRIVE_NAME") = "ทุกประเภท"
    ...
    Private Sub showPrice1_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Price1.Init
        ...
        Sql = "SELECT * FROM ListPrice_Car WHERE Category = 2 Order by ID"
        da = New OleDbDataAdapter(Sql, conn)
    ...
    Private Sub showCC1_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles CC1.Init
        ...
        Sql = "SELECT * FROM ListCC WHERE Category = 2 Order by ID"
        da = New OleDbDataAdapter(Sql, conn)
        da.Fill(ds, "LCC")
    ...
    Private Sub showPower1_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles power1.Init
        ...
        Sql = "SELECT * FROM ListPower WHERE Category = 2 Order by ID"
        da = New OleDbDataAdapter(Sql, conn)
        da.Fill(ds, "LPower")
    ...
    Private Function isMax(ByVal price As Double, ByVal type As Integer) As Boolean
        ...
        Select Case type
            Case 1 : sql = "SELECT max(Price2_Value) as Price FROM ListPrice_Car WHERE Category = 2 "

            Case 2 : sql = "SELECT min(Price1_Value) as Price FROM ListPrice_Car WHERE Category = 2"

            Case 3 : sql = "SELECT min(RYear_Value) as Price FROM ListYear "

            Case 4 : sql = "SELECT min(LYear_Value) as Price FROM ListYear "

            Case 5 : sql = "SELECT min(LCC_Value) as Price FROM ListCC WHERE Category = 2"

            Case 6 : sql = "SELECT max(RCC_Value) as Price FROM ListCC WHERE Category = 2"

            Case 7 : sql = "SELECT max(Rpower_Value) as Price FROM ListPower WHERE Category = 2 "

            Case 8 : sql = "SELECT min(Lpower_Value) as Price FROM ListPower WHERE Category = 2"
    ...
    Function checkStatus(ByVal status As Integer) As String
        ...
        If status <> 0 Then
            status_condition = " And (Bike_Status = " & status & ") "
    ...
    Function checkEngineType(ByVal engine_type As Integer) As String
        ...
        If engine_type <> 0 Then
            engine_type_condition = " And (Bike_EngineType = " & engine_type & ") "
    ...
    Function checkGear(ByVal gear_id As Integer) As String
        ...
        If gear_id <> 0 Then
            gear_condition = " And (Bike_Gears = " & gear_id & ") "
    ...
    Function checkColor(ByVal color_id As Integer) As String
        ...
        If color_id <> 0 Then
            color_condition = " And (Bike_Color = " & color_id & ") "
    ...
    Function checkColorMetalic(ByVal metalic_check As Boolean) As String
        ...
        If metalic_check Then
            metalic_condition = " And (Bike_Metalic = 1) "
    ...
    Function checkInsurrance(ByVal ins As Integer) As String
        ...
        If ins = 1 Then
            insurrance_condition = " And (Bike_Insurrance = 0) "
    ...
    Function checkAccident(ByVal acc As Integer) As String
        ...
        Select Case acc
            Case 0 : accident_condition = " And ((Bike_accident = 0)or(Bike_accident is null)) "
            Case 1 : accident_condition = ""
            Case 2
                accident_condition = " And (Bike_accident = 1) "
    ...
    Function checkPower(ByVal pow1 As Double, ByVal pow2 As Double) As String
        ...
        ElseIf (pow1 <> 0) And (pow2 = 0) Then
            ...
            If isMax(pow1, 8) Then
                power_condition = " And (Bike_Power <= " & pow1 + 1 & ") "
                Session("SearchCond") = Session("SearchCond") & "<= " & pow1 + 1 & "hp; "
            Else
                power_condition = " And (Bike_Power >= " & pow1 & ") "
        ...
        ElseIf (pow1 = 0) And (pow2 <> 0) Then
            ...
            If isMax(pow2, 7) Then
                power_condition = " And (Bike_Power >= " & pow2 - 1 & ") "
                Session("SearchCond") = Session("SearchCond") & ">= " & pow2 - 1 & "hp; "
            Else
                power_condition = " And (Bike_Power <= " & pow2 & ") "
        ...
        ElseIf (pow1 <> 0) And (pow2 <> 0) Then
            ...
            If pow1 < pow2 Then
                ...
                Else
                    If isMax(pow2, 7) Then
                        ...
                        Session("SearchCond") = Session("SearchCond") & ">= " & pow1 & "hp; "
                    Else
                        power_condition = " And (Bike_Power between " & pow1 & " and " & pow2 & ") "
            ...
            Else
                If isMax(pow2, 7) Then
                    ...
                    Session("SearchCond") = Session("SearchCond") & ">= " & pow2 - 1 & "hp; "
                Else
                    power_condition = " And (Bike_Power between " & pow2 & " and " & pow1 & ") "
    ...
    Function checkMile(ByVal mi1 As Double, ByVal mi2 As Double) As String
        ...
        ElseIf (mi1 <> 0) And (mi2 = 0) Then
            ...
            mile_condition = " And (Bike_Miles >= " & mi1 & ") "
        ...
        ElseIf (mi1 = 0) And (mi2 <> 0) Then
            ...
            If isMax(mi2, 9) Then
                mile_condition = " And (Bike_Miles >= " & mi2 - 1 & ") "
                Session("SearchCond") = Session("SearchCond") & ">= " & mi2 - 1 & "km; "
            Else
                mile_condition = " And (Bike_Miles <= " & mi2 & ") "
        ...
        ElseIf (mi1 <> 0) And (mi2 <> 0) Then
            ...
            If mi1 < mi2 Then
                ...
                Else
                    If isMax(mi2, 9) Then
                        ...
                        Session("SearchCond") = Session("SearchCond") & ">= " & mi1 & "km; "
                    Else
                        mile_condition = " And (Bike_Miles between " & mi1 & " and " & mi2 & ") "
            ...
            Else
                If isMax(mi2, 9) Then
                    ...
                    Session("SearchCond") = Session("SearchCond") & ">= " & mi2 - 1 & "km; "
                Else
                    mile_condition = " And (Bike_Miles between " & mi2 & " and " & mi1 & ") "
    ...
    Function checkShowPic(ByVal show_PIC As Integer) As String
        ...
        If show_PIC <> 0 Then
            showpic_condition = " And (Bike_Image1 is not null) "
        Else
            showpic_condition = ""
        End If
    ...
    Function checkShowPrice(ByVal show_Price As Integer) As String
        ...
        If show_Price <> 0 Then
            showprice_condition = " And (Bike_Price is not null) "
        Else
            showprice_condition = ""
        End If
    ...
    Function checkAcc() As String
        ...
        For i = 0 To accessory1.Items.Count - 1
            If accessory1.Items(i).Selected Then
                ...
                Session("SearchCond") = Session("SearchCond") & AccesoryDB.getBikeAccessName(accessory1.Items(i).Value) & "; "
                v_count = v_count + 1
        ...
        For i = 0 To accessory2.Items.Count - 1
            If accessory2.Items(i).Selected Then
                ...
                Session("SearchCond") = Session("SearchCond") & AccesoryDB.getBikeAccessName(accessory2.Items(i).Value) & "; "
                v_count = v_count + 1
        ...
        For i = 0 To accessory3.Items.Count - 1
            If accessory3.Items(i).Selected Then
                ...
                Session("SearchCond") = Session("SearchCond") & AccesoryDB.getBikeAccessName(accessory3.Items(i).Value) & "; "
                v_count = v_count + 1
        ...
        If acc1 <> "" Then
            ...
            accessory = "  AND (Bike.Bike_Id in (SELECT   CAccessory_BikeID FROM BAccessory WHERE (CAccessory_AccID in (" & accessory & "))group by CAccessory_BikeID having count(*) >= " & v_count & "))"
        Else
            accessory = ""
        End If
    ...
    Private Sub doSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles doSearch.Click
        ...
        'sql = "SELECT Bike.* FROM Bike INNER JOIN NoticeDetail ON Bike.Bike_Id = NoticeDetail.NoticeDetail_DataId INNER JOIN Notice ON NoticeDetail.NoticeDetail_NoticeId = Notice.Notice_Id INNER JOIN Customer ON Notice.Notice_MemberId = Customer.Customer_Id WHERE (Notice.Notice_Category = 2) AND (Notice.Notice_Show = 1)AND (NoticeDetail.Detail_delete IS NULL OR NoticeDetail.Detail_delete <> 1) " & sql
        ...
        Response.Redirect("search2_result_moto.aspx")
        'Response.Write(sql)
        'Response.Write(" WITH " & end_sql)
        'Response.Write(Session("SearchCond"))
    End Sub
End Class
