...
#End Region
    ...
    Private Sub doSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles doSearch.Click
        ...
        sql = "SELECT Car.* FROM Car INNER JOIN NoticeDetail ON Car.Car_Id = NoticeDetail.NoticeDetail_DataId "
        sql = sql & "INNER JOIN Notice ON NoticeDetail.NoticeDetail_NoticeId = Notice.Notice_Id "
        sql = sql & "INNER JOIN Customer ON Notice.Notice_MemberId = Customer.Customer_Id "
        sql = sql & "WHERE (Notice.Notice_Category = 1) AND (Notice.Notice_Show = 1) "
        sql = sql & "AND (Notice.Notice_Status IN (2, 3)) AND (DATEDIFF(Day, GETDATE(), Notice.Notice_StopDate) >= 0) "
        sql = sql & "AND (NoticeDetail.Detail_delete IS NULL OR NoticeDetail.Detail_delete <> 1) "
        sql = sql & "AND (NoticeDetail.Detail_show = 1) " & sql
        ...
        Response.Redirect("search2_result_car.aspx")
        'Response.Write(sql)
