<%@ Control Language="vb" AutoEventWireup="false" Codebehind="model_parts_list_request_search.ascx.vb" Inherits="autoonclick.model_parts_list_request_search" TargetSchema="http://schemas.microsoft.com/intellisense/ie5" %>
<P>
	...
	function brand_list(frm,BrandID){
		...
		c.options[0]= new Option('------------ กรุณาเลือกหมวดหมู่ ----------- ','0');
	...
	function model_list(frm,provID,modelID){
		...
		c.options[0]= new Option('------------ กรุณาเลือกประเภท ------------ ','0');
		if(provID=='0' || provID==''){
			c.options[0].selected=true;
		}else{
	...
	<%if session("brandIndex") = "" then%>
		brand_list('category','');
		model_list('parttype',0,'');
	<%else%>
		brand_list('category',<%=session("brandIndex")%>);
		model_list('parttype',<%=session("brandIndex")%>,<%=session("modelIndex")%>);
	<%end if%>
	</script>
	<FONT face="Tahoma">
