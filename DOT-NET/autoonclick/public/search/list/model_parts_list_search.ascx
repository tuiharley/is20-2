<%@ Control Language="vb" AutoEventWireup="false" Codebehind="model_parts_list_search.ascx.vb" Inherits="autoonclick.model_parts_list_search" TargetSchema="http://schemas.microsoft.com/intellisense/ie5" %>
<P>
	...
	function brand_list(frm,BrandID){
		...
		c.options[0]= new Option('------------ ทุกหมวดหมู่ ----------- ','0');
	...
	function model_list(frm,provID,modelID){
		...
		c.options[0]= new Option('------------ ทุกประเภท ------------ ','0');
		if(provID=='0' || provID==''){
			c.options[0].selected=true;
		}else{
