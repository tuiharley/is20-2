...
<%@ Page CodeBehind="parking2_example_01_moto.aspx.vb" Language="vb" AutoEventWireup="false" Inherits="autoonclick.parking2_example_01_moto" %>
...
//-->
	...
	<body onload="MM_preloadImages('../../images/main_table/offer_01.gif','../../images/main_table/search_01.gif','../../images/main_table/special_offer_01.gif','../../images/main_table/dealer_01.gif','../../images/main_table/company_01.gif','../../images/main_table/login_01_3.gif')">
		<form id="form1" name="form1" runat="server">
										...
										<tr>
											...
											<td>
												<div align="right">&#3610;&#3633;&#3609;&#3607;&#3638;&#3585;&#3649;&#3621;&#3657;&#3623;
													<span class="Font_black_bold"><%=request.form("saveBikeNum")%></span>
													&#3592;&#3634;&#3585; 
													&#3614;&#3639;&#3657;&#3609;&#3607;&#3637;&#3656;&#3607;&#3633;&#3657;&#3591;&#3627;&#3617;&#3604;
									...
									<table cellSpacing="0" cellPadding="0" border="0">
										<tr>
											...
											<td><input class="button" id="go1" onmouseover="document.all.go1.className='buttonover'" style="CURSOR: hand"
													onclick="doCommand(document.form1.commands1.value,<%=Request.Form("saveBikeNum")%>);" onmouseout="document.all.go1.className='button'"
									...
									<table class="All_Border_black" cellSpacing="0" cellPadding="0" width="118" border="0">
										<tr class="Font_black">
											<td width="25" bgColor="#999999">
												<div align="center">
													<input class="Font_black" onclick="checkAll(document.form1.carCheck,this.checked,<%=Request.Form("saveBikeNum")%>)" type="checkbox"
									...
									<table cellSpacing="0" cellPadding="0" border="0">
										<tr>
											...
											<td><input class="button" id="go2" onmouseover="document.all.go2.className='buttonover'" style="CURSOR: hand"
													onclick="doCommand(document.form1.commands2.value,<%=Request.Form("saveBikeNum")%>);" onmouseout="document.all.go2.className='button'"
			...
			<INPUT id="carNum" type="hidden" name="carNum" runat="server"> </TABLE></form>
		<script type='text/javascript' src="../../j_script/cookie.js"></script>
		<script language="javascript">
		
		//var sURL = unescape(window.location.pathname);
		var sURL = unescape('blank_moto.aspx');
		function refresh()
		{
			window.location.href = sURL;
		}
		function addCar(carid) {
					var x = getCookie('saveBike');
					setCookie('saveBike', x+'bike:'+carid+',', getExpDate(60, 0, 0),'/');
		}
		function addCarMsg1(carid,Msg) {
					var x = getCookie('saveBikeMsg');
					setCookie('saveBikeMsg', x+'bike:'+carid+':'+Msg+',', getExpDate(60, 0, 0),'/');
		}
		function addCarMsg(num) {
					var x;
					deleteCookie('saveBikeMsg','/');
					if(num!=1){
						for(n=0;n<num;n++){
							x = getCookie('saveBikeMsg');
							setCookie('saveBikeMsg', x+document.form1.M[n].value+document.form1.Msg[n].value+',', getExpDate(60, 0, 0),'/'); 
						}
					}else{
							setCookie('saveBikeMsg', document.form1.M.value+document.form1.Msg.value+',', getExpDate(60, 0, 0),'/'); 
					} 
					alert('ºÑ¹·Ö¡àÃÕÂºÃéÍÂ');
		}
		function incSaveNum() {
					var x = getCookie('saveBikeNum');
					if(x==''){
						setCookie('saveBikeNum', 1, getExpDate(60, 0, 0),'/');
					}else{
						setCookie('saveBikeNum', ++x, getExpDate(60, 0, 0),'/');
					}
		}
		function doCommand(i,num){
			switch (i){
				case '0':
					...
					break;
				case '1':
					deleteCookie('saveBike','/');
					deleteCookie('saveBikeNum','/');
					deleteCookie('saveBikeMsg','/');
						if(num!=1){
							for(n=0;n<num;n++){
								if(!document.form1.carCheck[n].checked){
									addCar(document.form1.carCheck[n].value);
									addCarMsg1(document.form1.carCheck[n].value,document.form1.Msg[n].value);
									incSaveNum();
								}
				...
				case '3':
					...
					if(num!=1){
						...
						if(document.form1.carCheck.checked){
									addCarMsg1(document.form1.carCheck.value,document.form1.Msg.value);
						}
					}	
					alert('Save Message');
					refresh();
				...
				case '4':
					if(num!=1){
						compare(document.getElementById("want_comp").value);
					}else{
		...
		function getCheckBox(TF,carid,curpage,num){		
			var tmptxt;
			if(TF){
				...
				if(num!=1){
					for(i=0;i<=<%=Request.Form("saveBikeNum")%>-1;i++){
		...
		function checkAll(field,checkEd,num){
				if(!checkEd){
					...
					document.form1.carNum.value=<%=Request.Form("saveBikeNum")%>;
				}
		}
		...
		function compare(comp){
			if(comp==''){
			  ...
			  autoonclick.parking2_example_01_moto.goComparePage(comp,UrlBack);
			}
