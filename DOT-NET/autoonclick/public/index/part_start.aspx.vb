...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        AjaxPro.Utility.RegisterTypeForAjax(GetType(part_start))
    ...
    <AjaxPro.AjaxMethod()> _
        ...
        Try
            'Car
            totalRow = myDS.Tables("Part").Rows.Count
            If totalRow > 0 Then
                ...
                For i = 1 To 5
                    For n = 1 To 2
                        If cnt <= totalRow Then
                            ...
                            myAds.id = myDS.Tables("Part").Rows(xResult - 1).Item("Car_Id")
                            myAds.Detailid = myDS.Tables("Part").Rows(xResult - 1).Item("NoticeDetail_Id")
                            myAds.Brand = myDS.Tables("Part").Rows(xResult - 1).Item("Brand_name")
                            myAds.Model = myDS.Tables("Part").Rows(xResult - 1).Item("Model_Name")
                            If myDS.Tables("Part").Rows(xResult - 1).Item("Status_Name") = "รถใหม่" Then
                                myAds.Status = "<span>New</span>"
                            Else
                                myAds.Status = "ปี " & checkNullDBstr(myDS.Tables("Part").Rows(xResult - 1).Item("Car_Year_Ord"))
                            End If

                            myAds.Price = FormatNumber(checkNullDBPrice(myDS.Tables("Part").Rows(xResult - 1).Item("Car_Price")), 2, , , TriState.True)
                            myAds.Image1 = checkNullDBstr(myDS.Tables("Part").Rows(xResult - 1).Item("Car_Image1"))
        ...
        txt &= "	<caption>""อะไหล่/ประดับยนต์"" ลงประกาศเมื่อเร็วๆนี้ บน AUTO-ONCLICK</caption>"
