...
Module NoticeDB
    ...
    Function getCatID(ByVal notice_id As Integer) As Integer
        ...
        Return package_id
    End Function
    Function getCatID2(ByVal detail_id As Integer) As Integer
        Dim myconn As New OleDbConnection(connectDB)
        myconn.Open()
        ...
        sql = "SELECT N.Notice_Category FROM Notice N,NoticeDetail ND WHERE N.Notice_Id = ND.NoticeDetail_NoticeId "
        sql = sql & " AND ND.NoticeDetail_Id  = " & detail_id
        mycommand = New OleDbCommand(sql, myconn)
        Dim Ans_ID As OleDbDataReader = mycommand.ExecuteReader()
        If Ans_ID.Read Then
            package_id = Ans_ID.Item("Notice_Category")
    ...
    Function getImgPath(ByVal cust_id As Integer, ByVal data_id As Integer, ByVal cat As Integer) As String
        ...
        Return dataid
    End Function
    Function getNoticeCode2(ByVal dataID As Integer, ByVal cat As Integer) As String
        Dim myconn As New OleDbConnection(connectDB)
        myconn.Open()
        ...
        sql = "SELECT Notice.Notice_Id ,NoticeDetail.NoticeDetail_Id,Dealer.Dealer_Notice_Abc FROM Notice INNER JOIN Dealer ON Notice.Notice_IsDealer = Dealer.DealerId INNER JOIN NoticeDetail ON Notice.Notice_Id = NoticeDetail.NoticeDetail_NoticeId WHERE (NoticeDetail.NoticeDetail_DataId = " & dataID & ") AND (Notice.Notice_Category = " & cat & ")  "
        mycommand = New OleDbCommand(sql, myconn)
        Dim Ans_ID As OleDbDataReader = mycommand.ExecuteReader()

        If Ans_ID.Read Then
            ans = Ans_ID.Item("Dealer_Notice_Abc") & Ans_ID.Item("NoticeDetail_Id")
        End If
        ...
        Return ans
    End Function
    Function getNoticeStopDate(ByVal notice_id As Integer) As DateTime
        ' Use In case of Private customer ONLY!!!
        ' One Notice One Detail !!
        ...
        If Ans_ID.Read Then
            If Not IsDBNull(Ans_ID.Item("Notice_StopDate")) Then
    ...
    Function getNoticeCode(ByVal notice_id As Integer) As String
        ...
        sql = "SELECT     Notice.Notice_Id, Dealer.Dealer_Notice_Abc "
        sql = sql & " FROM Notice INNER JOIN Dealer ON Notice.Notice_IsDealer = Dealer.DealerId"
        sql = sql & "  WHERE     (Notice.Notice_Id = " & notice_id & ")"

        mycommand = New OleDbCommand(sql, myconn)
        Dim Ans_ID As OleDbDataReader = mycommand.ExecuteReader()

        If Ans_ID.Read Then
            dataid = Ans_ID.Item("Dealer_Notice_Abc") & Ans_ID.Item("Notice_Id")
        End If
        ...
        Return dataid
    End Function
    Function getNoticeCodeDetail(ByVal data_id As Integer, ByVal cat As Integer) As String
        Dim myconn As New OleDbConnection(connectDB)
        myconn.Open()
        ...
        sql = "SELECT Notice.Notice_Id, NoticeDetail.NoticeDetail_Id, Dealer.Dealer_Notice_Abc FROM Notice INNER JOIN Dealer ON Notice.Notice_IsDealer = Dealer.DealerId INNER JOIN NoticeDetail ON Notice.Notice_Category = NoticeDetail.NoticeDetail_NoticeId WHERE (NoticeDetail.NoticeDetail_DataId = " & data_id & ") AND (Notice.Notice_Category = 1)  "
        mycommand = New OleDbCommand(sql, myconn)
        Dim Ans_ID As OleDbDataReader = mycommand.ExecuteReader()

        If Ans_ID.Read Then
            dataid = Ans_ID.Item("Dealer_Notice_Abc") & Ans_ID.Item("Notice_Id") & "D" & Ans_ID.Item("NoticeDetail_Id")
        End If
    ...
    Function getDealerCode(ByVal notice_id As Integer) As Integer
        ...
        If Ans_ID.Read Then
            dataid = checkNullDBNum(Ans_ID.Item("Notice_IsDealer"))
        End If
    ...
    Sub setOpenNotice(ByVal notice_id As Integer, ByVal open As Integer)
        Dim myconn As New OleDbConnection(connectDB)
        myconn.Open()
    ...
    End Sub
    Sub delNotice(ByVal notice_id As Integer)
        Dim myconn As New OleDbConnection(connectDB)
        myconn.Open()
        ...
        Try
            ...
            sql = "DELETE FROM JOB WHERE EXISTS (SELECT NULL FROM NOTICEDETAIL N WHERE N.NOTICEDETAIL_NOTICEID = " & notice_id & " AND N.NOTICEDETAIL_DATAID = JOB.JOB_ID)"
            mycommand.CommandText = sql
            mycommand.ExecuteNonQuery()

            sql = "DELETE FROM NOTICEDETAIL WHERE NoticeDetail_NoticeId = " & notice_id
            mycommand.CommandText = sql
            mycommand.ExecuteNonQuery()
    ...
    Function addNewNotice(ByVal v_cat As Integer, ByVal v_packageid As Integer, ByVal v_dealer As String, ByVal cust_id As Integer, ByVal v_paid As Integer, ByVal v_status As Integer, ByVal v_stop As String) As Integer
        ...
        'sql = sql & " VALUES (" & noticeid & ", " & v_cat & ", " & v_packageid & ", " & v_dealer & ", " & cust_id & ", " & v_paid & "," & v_status & ",getdate(), " & v_stop & ",null,0,null) "
        sql = sql & " VALUES (" & noticeid & ", " & v_cat & ", " & v_packageid & ", " & v_dealer & ", " & cust_id & ", " & v_paid & "," & v_status & ",getdate(),null, null,null,1,null,null) "
    ...
    Function getNotices(ByVal cust_id As Integer) As Array
        Dim notice(countNotices(cust_id)) As noticeS
        Dim myconn As New OleDbConnection(connectDB)
        myconn.Open()
        ...
        Return notice
    End Function
    Function getNoticeInfo(ByVal cust_id As Integer) As noticeS
        ...
        sql = "SELECT * FROM Notice WHERE Notice_MemberID  = " & cust_id
        mycommand = New OleDbCommand(sql, myconn)
        Dim Ans_ID As OleDbDataReader = mycommand.ExecuteReader()
        ...
        Return notice
    End Function
    Function getNoticeDataID(ByVal cust_id As Integer, ByVal data_id As Integer, ByVal cat As Integer) As Integer
        ...
        sql = sql & " AND     (Notice.Notice_Category = " & cat & ")"
        sql = sql & " AND     (NoticeDetail.NoticeDetail_DataId = " & data_id & ")"
        mycommand = New OleDbCommand(sql, myconn)
        Dim Ans_ID As OleDbDataReader = mycommand.ExecuteReader()

        If Ans_ID.Read() Then
            noticeID = Ans_ID.Item("Notice_Id")
    ...
    Function countOnlineOffline(ByVal cust_id As Integer, ByVal cat_type As String) As countOnOff
        ...
        Return result
    End Function
    Function countNoticeStatus(ByVal cust_id As Integer, ByVal cat_type As Integer) As countStatus
        Dim myconn As New OleDbConnection(connectDB)
        myconn.Open()
        ...
        sql = sql & " FROM          Notice a"
        sql = sql & " WHERE      (Notice_Status = 1) AND notice_memberid = " & cust_id & " AND notice_category = " & cat_type & ") AS regist,"
        sql = sql & " (SELECT     COUNT(*)"
        sql = sql & " FROM          Notice a"
        sql = sql & " WHERE      (Notice_Status = 2) AND notice_memberid = " & cust_id & " AND notice_category = " & cat_type & ") AS online,"
        sql = sql & " (SELECT     COUNT(*)"
        sql = sql & " FROM          Notice a"
        sql = sql & " WHERE      (Notice_Status = 3) AND notice_memberid = " & cust_id & " AND notice_category = " & cat_type & ") AS warn,"
        sql = sql & " (SELECT     COUNT(*)"
        sql = sql & " FROM          Notice a"
        sql = sql & " WHERE      (Notice_Status = 4) AND notice_memberid = " & cust_id & " AND notice_category = " & cat_type & ") AS stop,"
        sql = sql & " (SELECT     COUNT(*)"
        sql = sql & " FROM          Notice a"
        sql = sql & " WHERE      (Notice_Status = 5) AND notice_memberid = " & cust_id & " AND notice_category = " & cat_type & ") AS offline"

        mycommand = New OleDbCommand(sql, myconn)
        Dim Ans_ID As OleDbDataReader = mycommand.ExecuteReader()

        If Ans_ID.Read() Then
            result.regist = Ans_ID.Item("regist")
            result.online = Ans_ID.Item("online")
            result.warn = Ans_ID.Item("warn")
            result.stopped = Ans_ID.Item("stop")
            result.offline = Ans_ID.Item("offline")
        End If
    ...
    End Function
    Function getDealerNotices(ByVal cust_id As Integer, ByVal cat_type As String, ByVal sortBy As Integer) As Queue
        Dim myNoticeDealer As noticeS
        Dim result As New Queue
        Dim myconn As New OleDbConnection(connectDB)
        myconn.Open()
        ...
        sql = sql & " FROM         CategoryType INNER JOIN"
        sql = sql & " Category ON CategoryType.CategoryType = Category.Category_CategoryType INNER JOIN"
        sql = sql & " Notice ON Category.Category = Notice.Notice_Category INNER JOIN"
        sql = sql & " NoticeDetail ON Notice.Notice_Id = NoticeDetail.NoticeDetail_NoticeId INNER JOIN"
        sql = sql & " Job ON NoticeDetail.NoticeDetail_DataId = Job.Job_Id INNER JOIN"
        sql = sql & " Job_Position ON Job.Job_Pos = Job_Position.Position_ID"

        sql = sql & " WHERE  " & cat_type
        sql = sql & " AND (Notice.Notice_MemberId = " & cust_id & ") "
        Select Case sortBy
            Case 1
                sql = sql & " ORDER BY Notice_StartDate DESC"
            Case 2
                sql = sql & " ORDER BY Notice_StopDate"
            Case 3
                sql = sql & " ORDER BY NoticeDetail.Detail_show DESC"
            Case 4
                sql = sql & " ORDER BY Job_Position.Position_Name "
        End Select
        mycommand = New OleDbCommand(sql, myconn)
        Dim Ans_ID As OleDbDataReader = mycommand.ExecuteReader()
