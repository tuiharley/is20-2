...
#Region " Web Form Designer Generated Code "
    ...
    Protected WithEvents openClose As System.Web.UI.WebControls.Label
    ...
    Protected WithEvents JPosDate As System.Web.UI.WebControls.Label
    Protected WithEvents topicTxt As System.Web.UI.WebControls.Label
    Protected WithEvents delAdsCmd As System.Web.UI.WebControls.Label
...
#End Region
    ...
    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Dim curData As Integer = Request.QueryString("curData")
        ...
        Else
            ...
            detailID = NoticeDetailDB.getNoticeDetailID(noticeid)
            notice = NoticeDetailDB.getNoticeDetailInfo(detailID)

            initLink(myjob.Job_Id, notice)
            showCustInfo(Session("cust_id"))
            showHead(Session("cust_id"), myjob, myNotice)
            showData(myjob)
            upPage.Text = showNextPage(curData, Session("allCar"), noticeid)
        ...
        AjaxPro.Utility.RegisterTypeForAjax(GetType(job3_detail))
    End Sub

    Private Sub initLink(ByVal jobID As Integer, ByVal myDetail As noticeDetail)
        editCarData.NavigateUrl = "job4_edit1_datas.aspx?jobID=" & jobID

        If myDetail.Detail_show = 1 Then
            openClose.Text = "<td width='113' height='20' style='CURSOR: hand' onClick='showJob(" & myDetail.id & ");'><u><div align='center' id='delCar'>ปิดประกาศ</div></u></td>"
        Else
            openClose.Text = "<td width='113' height='20' style='CURSOR: hand' onClick='showJob(" & myDetail.id & ");'><u><div align='center' id='delCar'>เปิดประกาศ</div></u></td>"
        End If

        changeContact.NavigateUrl = "../change_contact/change_contact_datas.aspx"

        PrintThis.NavigateUrl = "../../../print_form/print_job_detail.aspx?jobid=" & jobID
        PrintThis.Target = "_blank"


        delAdsCmd.Text = "<font class='fontBl' id='delAd' onmouseover=""document.all.delAd.className='fontBl1'"" style=""CURSOR: hand"""
        delAdsCmd.Text = delAdsCmd.Text & "onclick=""deleteAds(" & myDetail.NoticeId & ");"" onmouseout=""document.all.delAd.className='fontBl'"">ลบประกาศ</font>"
    ...
    End Sub
    Private Sub showHead(ByVal dealerid As Integer, ByVal myJob As Job, ByVal myNotice As noticeS)
    ...
    End Sub
    Private Function showTopic(ByVal show As Boolean, ByVal b As String, ByVal myNotice As noticeS) As String
        ...
        topic = topic & "<td width='526'><strong>ประกาศตำแหน่งงาน: "
        topic = topic & b & " / วันที่ลงประกาศ: "

        If myNotice.status > 1 Then
            topic = topic & myNotice.startDate.ToString("dd-MM-yyyy")
        End If

        topic = topic & "</strong></td><td width='10'></td>"
    ...
    Private Function showNextPage(ByVal curData As Integer, ByRef allData As Queue, ByVal noticeid As Integer) As String
        ...
        Else
            ...
            x = x & "<td><a href='job3_detail.aspx?curData=" & curData - 1 & "&jobId=" & carid.DataId & "&noticeid=" & noticeid & "' class='Font_black'><<</td>"
        End If
    ...
    End Function
    <AjaxPro.AjaxMethod()> _
    Public Function displayT(ByVal DataId As Integer, ByVal display As Boolean) As String
        Dim myQ As New Queue
        Dim tmp As Position
        Dim txt As String
        Dim txt2 As String
        ...
        myJob = JobDB.getJob(DataId)
        myQ = JobPositionDB.getPosition(myJob.Job_Pos)
        For Each tmp In myQ
            txt = tmp.Position_Name
        Next


        myNotice = NoticeDB.getNoticesInfoD(NoticeDB.getNoticeDataID(Session("cust_id"), DataId, 6))
        txt2 = showTopic(display, txt, myNotice)


        Return txt2
    End Function
    <AjaxPro.AjaxMethod()> _
    Public Function deleteAdsAJX(ByVal noticeid As Integer) As String
    ...
    <AjaxPro.AjaxMethod()> _
    Public Function getNotice(ByVal noticeid As Integer) As Object

        Return NoticeDB.getNoticesInfoD(noticeid)
    End Function
End Class
