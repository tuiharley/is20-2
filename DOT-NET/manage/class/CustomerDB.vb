...
Module CustomerDB
    ...
    Function getRanCust(ByVal cust_id As Integer, ByVal Qtype As String) As Queue
        ...
        sql = "SELECT TOP 250  Customer.Customer_Id, Customer.Customer_Username, Customer.Customer_Logo, District.District_Name, "
    ...
    End Function
    Function getCustomers(ByVal Cname As String, ByVal selType As Integer) As Queue
        Dim myconn As New OleDbConnection(connectDB)
        myconn.Open()
        ...
        Select Case selType
            Case 1
                ...
                'sql = sql & " WHERE (Customer_Username like '%" & Cname & "%')"

                sql = "SELECT     C.Customer_Id, C.Customer_Type, C.Customer_Username, C.Customer_Passwd, C.Customer_Title, C.Customer_Name, C.Customer_Surname, "
                sql = sql & " C.Customer_ContactName, C.Customer_Homepage, C.Customer_Email, C.Customer_Phone1, C.Customer_Phone2, C.Customer_Fax, C.Customer_Logo, "
                sql = sql & " C.Customer_Map, C.Customer_Address, C.Customer_District, C.Customer_Province, C.Customer_DateStart, C.Customer_DateStop, "
                sql = sql & " C.Customer_TimeStart, C.Customer_TimeStop, C.Customer_Inquire, C.Customer_Notice, C.Customer_Accept, C.Customer_PostDate"
                sql = sql & " , d.amount"
                sql = sql & ",("
                sql = sql & " Select count(notice_status) "
                sql = sql & " from notice n "
                sql = sql & " where n.notice_memberid = C.customer_id "
                sql = sql & " and n.notice_status in (2,3) "
                sql = sql & ") online "
                sql = sql & ",( "
                sql = sql & " select count(notice_status) "
                sql = sql & " from notice n "
                sql = sql & " where n.notice_memberid = C.customer_id "
                sql = sql & " and n.notice_status in (1,5) "
                sql = sql & " ) offline "
                sql = sql & " FROM         Customer C LEFT OUTER JOIN"
                sql = sql & " (SELECT     SUM(inv.package_price) amount, n.notice_memberid"
                sql = sql & " FROM          invoice inv, notice n"
                sql = sql & " WHERE      inv.paid = 1 AND (inv.cancel = 0 OR"
                sql = sql & " inv.cancel IS NULL) AND inv.notice_id = n.notice_id"
                sql = sql & " GROUP BY n.notice_memberid) d ON C.Customer_Id = d.notice_memberid"
                sql = sql & " WHERE     (C.Customer_Username LIKE '%" & Cname & "%')"
                sql = sql & " AND Customer_Title <> 99"
                sql = sql & " GROUP BY C.Customer_Id, C.Customer_Type, C.Customer_Username, C.Customer_Passwd, C.Customer_Title, C.Customer_Name, C.Customer_Surname, "
                sql = sql & " C.Customer_ContactName, C.Customer_Homepage, C.Customer_Email, C.Customer_Phone1, C.Customer_Phone2, C.Customer_Fax, C.Customer_Logo, "
                sql = sql & " C.Customer_Map, C.Customer_Address, C.Customer_District, C.Customer_Province, C.Customer_DateStart, C.Customer_DateStop, "
                sql = sql & " C.Customer_TimeStart, C.Customer_TimeStop, C.Customer_Inquire, C.Customer_Notice, C.Customer_Accept, C.Customer_PostDate, d.amount"
            Case 2
                sql = "SELECT Customer_Id, Customer_Type, Customer_Username, Customer_Passwd, Customer_Title, Customer_Name, Customer_Surname, "
                sql = sql & " Customer_ContactName, Customer_Homepage, Customer_Email, Customer_Phone1, Customer_Phone2, Customer_Fax, Customer_Logo, "
                sql = sql & " Customer_Map, Customer_Address, Customer_District, Customer_Province, Customer_DateStart, Customer_DateStop, Customer_TimeStart, "
                sql = sql & " Customer_TimeStop , Customer_Inquire, Customer_Notice, Customer_Accept,Customer_PostDate"
                sql = sql & " FROM  customer "
                sql = sql & " WHERE (Customer_Name like '%" & checkNullstr(Cname) & "%')"
            Case 3
            ...
            Case 99
                ...
                sql = sql & " inv.cancel IS NULL) AND inv.notice_id = n.notice_id"
                sql = sql & " GROUP BY n.notice_memberid) d ON C.Customer_Id = d.notice_memberid"
                sql = sql & " WHERE     (C.Customer_Id = " & Cname & ")"
                sql = sql & " AND Customer_Title <> 99"
                sql = sql & " GROUP BY C.Customer_Id, C.Customer_Type, C.Customer_Username, C.Customer_Passwd, C.Customer_Title, C.Customer_Name, C.Customer_Surname, "
                sql = sql & " C.Customer_ContactName, C.Customer_Homepage, C.Customer_Email, C.Customer_Phone1, C.Customer_Phone2, C.Customer_Fax, C.Customer_Logo, "
                sql = sql & " C.Customer_Map, C.Customer_Address, C.Customer_District, C.Customer_Province, C.Customer_DateStart, C.Customer_DateStop, "
        ...
        While Ans_ID.Read
            ...
            customer.online = String_chk.checkNullDBNum(Ans_ID.Item("online"))
            customer.offline = String_chk.checkNullDBNum(Ans_ID.Item("offline"))
        ...
        Return myQ
    End Function
    Function getCustomers(ByVal CTyp As Integer, ByVal Cname As String, ByVal selType As Integer) As Queue
        Dim myconn As New OleDbConnection(connectDB)
        myconn.Open()
        ...
        Select Case selType
            Case 1
                ...
                sql = sql & " WHERE     (C.Customer_Username LIKE '%" & Cname & "%')"
                sql = sql & " AND Customer_Title <> 99"
                sql = sql & " AND C.Customer_Type =" & CTyp
                sql = sql & " GROUP BY C.Customer_Id, C.Customer_Type, C.Customer_Username, C.Customer_Passwd, C.Customer_Title, C.Customer_Name, C.Customer_Surname, "
                sql = sql & " C.Customer_ContactName, C.Customer_Homepage, C.Customer_Email, C.Customer_Phone1, C.Customer_Phone2, C.Customer_Fax, C.Customer_Logo, "
                sql = sql & " C.Customer_Map, C.Customer_Address, C.Customer_District, C.Customer_Province, C.Customer_DateStart, C.Customer_DateStop, "
                sql = sql & " C.Customer_TimeStart, C.Customer_TimeStop, C.Customer_Inquire, C.Customer_Notice, C.Customer_Accept, C.Customer_PostDate, d.amount"
    ...
    Function getA_customer(ByVal cust_id As Integer) As A_Customer
        ...
        Return customer
    End Function
    Function getPriSaleHis(ByVal cust_id As Integer) As Queue
        Dim myconn As New OleDbConnection(connectDB)
        myconn.Open()
        ...
        Dim sql As String
        Dim myQ As New Queue
        Dim customer As PriSaleHis

        sql = "select n.notice_category ,n.notice_memberid,count(*) as num"
        sql = sql & ",	("
        sql = sql & " Select sum(inv.package_price)"
        sql = sql & " from invoice inv"
        ...
        sql = sql & " AND n2.notice_category = n.notice_category"
        sql = sql & " GROUP BY n2.notice_category,n2.notice_id"
        sql = sql & " )"
        sql = sql & " ) as amount"
        sql = sql & " from notice n"
        sql = sql & " where n.notice_memberid = " & cust_id
        sql = sql & " GROUP BY n.notice_category,n.notice_memberid"
        mycommand = New OleDbCommand(sql, myconn)
        Dim Ans_ID As OleDbDataReader = mycommand.ExecuteReader()

        While Ans_ID.Read
            customer.notice_category = String_chk.checkNullDBNum(Ans_ID.Item("notice_category"))
            customer.notice_memberid = String_chk.checkNullDBNum(Ans_ID.Item("notice_memberid"))
            customer.num = String_chk.checkNullDBNum(Ans_ID.Item("num"))
            customer.amount = String_chk.checkNullDBPrice(Ans_ID.Item("amount"))
            myQ.Enqueue(customer)
    ...
    End Structure
    Structure PriSaleHis
        Dim notice_category As Integer
        Dim notice_memberid As Integer
        Dim num As Integer
