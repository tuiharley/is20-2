...
#End Region
    ...
    Private Sub showHead(ByVal dealerid As Integer)
        ...
        Else
            contact.Text = contact.Text & " เวลาทำการ: " & CustomerPaymentDB.getDateOpen(myCust.dateStart) & " - " & CustomerPaymentDB.getDateOpen(myCust.dateStop)
        End If
        If myCust.timeStart <> 0 And myCust.timeStop <> 0 Then
            contact.Text = contact.Text & " : " & CustomerPaymentDB.getTimeOpen(myCust.timeStart) & " - " & CustomerPaymentDB.getTimeOpen(myCust.timeStop) & " น."
        End If
