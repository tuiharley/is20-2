<%@ Control Language="vb" AutoEventWireup="false" Codebehind="private_model_parts2_list_puadmin.ascx.vb" Inherits="manage.private_model_parts2_list_puadmin" TargetSchema="http://schemas.microsoft.com/intellisense/ie5" %>
<P>
	...
	function brand_list(frm,BrandID){
		...
		c.options[0]= new Option('------------ เลือกหมวดหมู่ ----------- ','0');
	...
	function model_list(frm,provID,modelID){
		...
		c.options[0]= new Option('------------ เลือกประเภท ------------ ','0');
		if(provID=='0' || provID==''){
			c.options[0].selected=true;
		}else{
