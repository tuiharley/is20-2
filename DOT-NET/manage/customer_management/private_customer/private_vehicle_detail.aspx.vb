Public Class private_vehicle_detail
...
#End Region
    ...
    Private Sub checkShow()
        ...
        'form_internal_status_changing_of_ad_or_pack.aspx?page=adList&noticeid=" & A_customer_notices(i).id & "&showNotice=" & show & "
        If showNotice <> "" Then

            If showNotice = "yes" Then
                'Response.Redirect("form_internal_status_changing_of_ad_or_pack.aspx?page=car&noticeid=" & noticeid & "&showNotice=OFF" & "&curPos=" & curPos & "&cust_id=" & cust_id)
                'NoticeDB.setOpenNotice(noticeid, 1)
            Else
                'Response.Redirect("form_internal_status_changing_of_ad_or_pack.aspx?page=car&noticeid=" & noticeid & "&showNotice=ON" & "&curPos=" & curPos & "&cust_id=" & cust_id)
    ...
    Private Sub setLink(ByVal show As String, ByVal noticeid As Integer, ByVal cust_id As Integer, ByVal curPos As Integer)
        ...
        editAd.NavigateUrl = "private_edit1_car_datas.aspx?from=start&noticeid=" & noticeid & "&cust_id=" & cust_id & "&curPos=" & curPos
        del_ad.NavigateUrl = "private_vehicle_detail.aspx?del=yes&noticeid=" & noticeid & "&cust_id=" & cust_id & "&curPos=" & curPos

        '--------------------- form_internal_status_changing_of_ad_or_pack.aspx?page=car&noticeid=" & noticeid & "&showNotice=OFF" & "&curPos=" & curPos & "&cust_id=" & cust_id)
        show_ad.NavigateUrl = "form_internal_status_changing_of_ad_or_pack.aspx?page=car&showNotice=" & tmp2 & "&noticeid=" & noticeid & "&cust_id=" & cust_id & "&curPos=" & curPos
        '--------------------------------------------------page=adList&noticeid=" & A_customer_notices(i).id & "&showNotice=" & show & "' 
    ...
    Private Sub showdetail(ByVal notice_id As Integer, ByVal cust_id As Integer)
        ...
        brand.Text = BrandDB.getBrandName(myCar.Brand) & "/" & ModelDB.getModelName(myCar.Model)
