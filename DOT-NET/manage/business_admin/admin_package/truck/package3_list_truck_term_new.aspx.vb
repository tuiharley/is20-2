...
Public Class package3_list_truck_term_new
...
#End Region
    ...
    Const category As Integer = 3

    Private Sub Page_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        'Put user code to initialize the page here
        AjaxPro.Utility.RegisterTypeForAjax(GetType(showBrands))
        AjaxPro.Utility.RegisterTypeForAjax(GetType(package3_list_truck_term_new))
        Response.Expires = 0
    ...
    Private Function showEmpty(ByVal myNoticeDealer As numNoticeDealer, ByVal noticeStatus As Integer) As String
        ...
        Else
            sql = sql & "<a href='package5_offer1_truck_datas.aspx?noticeid=" & myNoticeDealer.noticeId & "' class='A80'>"
        End If
        sql = sql & "&#3621;&#3591;&#3611;&#3619;&#3632;&#3585;&#3634;&#3624;</a>"
        sql = sql & "</div>"
        sql = sql & "</td>"
    ...
    Public Function showTerm(ByVal cat As Integer) As String
        ...
        sql = sql & " 											<td width=""526""><strong>รายการรถบรรทุก:"
        ...
        If myNoticeDealer.rowNum > 0 Then ' ===========ในกรณีที่มีการลงประกาศไว้แล้ว ===============
            ...
            sql = sql & " <select class='Font_black' style='WIDTH: 295px' name='brand' id='brand' > "
            ...
            sql = sql & "  <input type='text' class='Font_black' style='WIDTH: 294px' name='model' id='model' value='ทุกรุ่น'> "
    ...
    Private Function showButton1(ByVal myNoticeDealer As numNoticeDealer) As String
        ...
        If myNoticeDealer.rowNum < myNoticeDealer.totalNum And myNoticeDealer.status <> 5 Then
            sql = sql & "<A class=A80 href='package5_offer1_truck_datas.aspx?noticeid=" & myNoticeDealer.noticeId & "' >&#3621;&#3591;&#3611;&#3619;&#3632;&#3585;&#3634;&#3624;</A>"
        Else
    ...
    Private Function showCars(ByVal myDetail As noticeDetail, ByVal n As Integer) As String
        ...
        myCar = TruckDB.getTruck(myDetail.DataId)
        ' =====================  Start ==========================
        sql = sql & "<table cellSpacing=0 cellPadding=0 width=693 border=0 >"
        sql = sql & "<tr>"
        ...
        sql = sql & "                       <td width=149>"
        sql = sql & "                         <div align=center>"
        sql = sql & BrandDB.getBrandName(myCar.Brand) & "/ " & myCar.Model
        sql = sql & "</div></td>"
        sql = sql & "                       <td width=5 "
        sql = sql & "                       background=../../../images/separator/g_5.gif> "
        sql = sql & "                       </td>"
        sql = sql & "                       <td width=83>"
        sql = sql & "                         <div align=center>"
        sql = sql & CarTypeDB.getStatusName(myCar.Type, 3)
        sql = sql & "</div></td>"
        sql = sql & "                       <td width=5 "
        sql = sql & "                       background=../../../images/separator/g_5.gif> "
        sql = sql & "                       </td>"
        ...
        sql = sql & "                       <td width=41>"
        '=========== small pic ==========================
        sql = sql & "                         <div align=center><A href='package4_detail_truck.aspx?carid=" & myDetail.DataId & "&detailid=" & myDetail.id & "&noticeid=" & myDetail.NoticeId & "&curData=" & n & "' >"
        ...
        sql = sql & "                       background=../../../images/separator/g_5.gif> "
        sql = sql & "                       </td>"
        sql = sql & "                       <td width=60><A class=A60 href=""package4_edit_truck_datas.aspx?from=list&car=" & myDetail.DataId & "&detailid=" & myDetail.id & "&notice_id=" & myDetail.NoticeId & "&curData=" & n & """ >&#3649;&#3585;&#3657;&#3652;&#3586;</A></td>"
        sql = sql & "                       <td width=3></td></tr>"
        sql = sql & "                     <tr>"
        ...
        sql = sql & "                       <td>"
        sql = sql & "                         <div align=center><A  href='package4_detail_truck.aspx?carid=" & myDetail.DataId & "&detailid=" & myDetail.id & "&noticeid=" & myDetail.NoticeId & "&curData=" & n & "' >"
        sql = sql & "<IMG height=45 src='"
        ' ========================= car Pic ==========================
        If myCar.Image1 <> "" Then
            sql = sql & "/userData/" & CustomerDB.getUserName(Session("cust_id")) & "/" & NoticeDB.getImgPath(Session("cust_id"), myCar.ID, 3) & "/" & myCar.Image1
        Else
            sql = sql & "../../../images/objects/no_image_60x45.gif"
        End If

        sql = sql & "' width=60 border=0 ></A></div></td>"
        sql = sql & "                       <td width=5 "
        sql = sql & "                       background=../../../images/separator/g_5.gif> "
        sql = sql & "                       </td>"
        '===============  remark ข้อความ ==========================================
   ...
   Public Function saveRemark(ByVal carid As Integer, ByVal remark As String) As String
        ...
        If remark <> "ข้อความของคุณ..." Then
            TruckDB.editTruckRemark(carid, remark)
            result = "ok"
        Else
            result = "not"
        End If
   ...
   Public Function setDetailDelete(ByVal detailid As Integer) As String
    ...
    Function checkModel(ByVal m_name As String) As String
        ...
        If (m_name <> "") And (m_name <> "ทุกรุ่น") Then
    ...
    Function checkOrderBy(ByVal orderby As String) As String
        ...
        Select Case orderby
            Case "price"
                Order_By_Condition = " ORDER BY Truck.Truck_Price"
                KeyOrder_By = "Truck_Price"
            Case "year"
                Order_By_Condition = " ORDER BY Truck.Truck_Year_Ord DESC"
                KeyOrder_By = "Truck_Year_Ord DESC"
            Case "brand"
                Order_By_Condition = " ORDER BY Brand.Brand_Name,Truck_model"
                KeyOrder_By = "Brand_Name"
            Case "post"
                Order_By_Condition = " ORDER BY Truck.Truck_Post DESC"
                KeyOrder_By = "Truck_Post DESC"
            Case "status"
                Order_By_Condition = " ORDER BY NoticeDetail.Detail_show"
                KeyOrder_By = "Detail_show"
            Case "carType"
                Order_By_Condition = " ORDER BY Truck.Truck_Type DESC"
                KeyOrder_By = "Truck_Type DESC"
            Case "-"
                Order_By_Condition = "-"
                KeyOrder_By = "-"
    ...
    <AjaxPro.AjaxMethod()> _
    Public Function showSelectedC(ByVal brand As Integer, ByVal status As Integer, ByVal model As String, ByVal yearst As Integer, ByVal yearend As Integer, ByVal carType As Integer, ByVal pricest As Double, ByVal priceend As Double, ByVal orderBy As String) As String
        Dim sql As String
    ...
    Public Function showB1() As String
        ...
        Dim myNoticeDealer As numNoticeDealer
        myNoticeDealer = NoticeDB.countDealerNotices(Session("cust_id"), 3)
