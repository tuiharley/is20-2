...
#End Region
    ...
    Private Function showNotice(ByVal myNotice As noticeS) As String
        ...
        sql = sql & " </td>"
        sql = sql & " <td width='5'></td>"
        sql = sql & " <td width='75'><div align='center'>&#3647; " & PackageDB.getPackagePrice(myNotice.pacakgeId).ToString("#,###.##")
        sql = sql & " </div>"
        sql = sql & " </td>"
        sql = sql & " <td width='5'></td>"
        ...
        sql = sql & " </td>"
        sql = sql & " <td width='5'></td>"
        sql = sql & " <td width='46'><div align='center'><a href='ads_sorted_by_OffOn.htm' class='A_black_underline'>" & NoticeDetailDB.countDealerNoticeDetailID(myNotice.id) & "</a></div>"
        sql = sql & " </td>"
        sql = sql & " <td width='5'></td>"
        ...
        '------------------ลงประกาศเพิ่ม ----------------------------------
        If (myNotice.status = 1 Or myNotice.status = 5) Or (NoticeNum = PackageDB.getPackageWeek(myNotice.pacakgeId)) Then
