...
<font face="MS Sans Serif" size="4" color="#0000FF">เลือกวาระที่ต้องการให้แสดงผลการลงคะแนน<br><BR></font>
	...
	do until rs.eof or rs.bof
 	...
 	if rs1.eof or rs1.bof then 
			 If rs(2) > time()  then
		 		if subject <> left(rs(1),10) then 
					response.write "<li><a target='_parent' href=vote_result2.asp?question_id="&rs(0)&"&sub_question_id=0><font class=f1>"&rs(1)&"</font></a></li>"
				else
				response.write "<ul><li><a target='_parent' href=vote_result2.asp?question_id="&rs(0)&"&sub_question_id=0><font class=f1>"&mid(rs(1),11)&"</font></a></li></ul>"
				end if
			else
				if subject <> left(rs(1),10) then 
					response.write "<li><a target='_parent' href=vote_result2.asp?question_id="&rs(0)&"&sub_question_id=0><font class=f1 >"&rs(1)&"</font></a>&nbsp;<font size='-1' color='#E31A1A'>Complete!</font></li>"
				else
				response.write "<ul><li><a target='_parent' href=vote_result2.asp?question_id="&rs(0)&"&sub_question_id=0><font class=f1 >"&mid(rs(1),11)&"</font></a></li></ul>"
				end if
			end if
...
response.write "<ul>"
do until rs1.eof
response.write "<li><font class=f1><a target='_parent' href=vote_result2.asp?question_id="&rs(0)&"&sub_question_id="&rs1(0)&"><font class=f1>"&rs1(1)&"</font></a></li>"
rs1.movenext
loop
