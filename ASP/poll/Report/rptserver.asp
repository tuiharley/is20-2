<%
'**********************************************************************************************
'     Rptserver.asp ver8.5 - Modification Date - (Thursday July 12th,2001  10:40am PST) 
'**********************************************************************************************
...
' 05/25/01
' Added the following:
' Check for the existance of the objects session("oRpt"), session("oApp") and session("oPageEngine")
' to prevent errors when rptserver is run after these objects have been cleaned up.


 On Error Resume Next

If NOT isobject(session("oRpt")) or NOT isobject(session("oApp")) or NOT isobject(session("oPageEngine")) then
	response.end
end if
...
Case "GET_PG"
	...
	Select Case VFMT
		...
		Case "HTML_PAGE"
			session("oPageEngine").ImageOptions = 1
			response.binarywrite goPage.Renderhtml(1,0,3,request.ServerVariables("SCRIPT_NAME"),8209)
...
Case "RFSH"
	...
	Case "HTML_PAGE"
	session("oPageEngine").ImageOptions = 1
	response.binarywrite goPage.Renderhtml(1,0,1,request.ServerVariables("SCRIPT_NAME"),8209)
