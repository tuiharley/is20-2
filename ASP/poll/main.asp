	<title>TT&T - ระบบการประชุมผู้ถือหุ้น</title>
<frameset rows="75,*" framespacing="0" border="0" frameborder="0">
<% 
if (request("login") = "chairman" and request("pw") = "manager") or (request("login") = "staff" and request("pw") = "staff")  or (request("login") = "admin" and request("pw") = "admin") or (request("login") = "president" and request("pw") = "manager") or (request("login") = "management" and request("pw") = "manager") then
	...
	elseif request("login") = "admin" then
		...
		response.write "<frame src=home.asp name=bottom scrolling=auto>"
	elseif request("login") = "management"   then
