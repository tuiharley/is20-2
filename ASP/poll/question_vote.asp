...
<table border="1" cellspacing="1" width="100%" id="AutoNumber1" bgcolor="#9900FF" height="26" bordercolor="#9900FF">
  <tr>
    ...
    <b>
    <span lang="th">
    <font face="MS Sans Serif" size="4" color="#0000FF"><div align="center">เลือกหัวข้อวาระการออกเสียงลงคะแนน</div></font></b></td>
  </tr>
</table>
<!--- &nbsp;&nbsp;&nbsp; <b><font class=f1>
...
<% 	if rs1.eof or rs1.bof then 
		 If rs(3) > time()  then%>
		<li><a target="right" href=committee_votetest.asp?question_id=<%=rs(0) %>&sub_question_id=0><font class=f1><%=rs(1) %></font></a>
		<% Else  %>
	...
	do until rs1.eof
		If rs(3) > time()  then%>
		<li><a target="right" href=committee_votetest.asp?question_id=<%=rs(0) %>&sub_question_id=<%= rs1(0) %>><font class=f1><%=rs1(1) 	%></font></a>			
		<% 
		else%>
