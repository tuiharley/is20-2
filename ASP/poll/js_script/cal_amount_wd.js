...
//การทำงานเมื่อ Radio ir_Paid_by เปลี่ยนแปลง
function ir_Paid_by_onChick()
    {
	with (document.f_Withdraw)
	    {
		if (ir_Paid_by[0].checked)
		    {
			alert("Radio is Cash");
   			sl_bank.onFocus = blur();
   			it_Branch.onFocus = blur();
   			it_Chq_no.onFocus = blur();
   			it_Chq_date.onFocus = blur();
		    }
		else
		    {
			alert("Radio is Cheque");
   			sl_bank.onFocus = focus();
   			it_Branch.onFocus = focus();
   			it_Chq_no.onFocus = focus();
   			it_Chq_date.onFocus = focus();
		    }
            }
    }
