...
<body onLoad="document.textboxx.stockholderid.focus();">
...
response.write "</b></font>" 
  ...
  rs.open sql,conn,1,3
	...
	rs.close
%>
<form  name="textboxx" action="vote.asp?question_id=<%=request("question_id")%>&sub_question_id=<%=request("sub_question_id")%>"  method="post">
...
หมายเลยลำดับของผู้ถือหุ้น 
<input type="text" name="stockholderid" size="15"> 
<table width="100%"><tr>
  <td><hr>
</td>
</tr></table>
...
sql = sql&" AND ( VOTE.STATUS = YES) AND ((Vote.sub_question_id)="&request("sub_question_id")&") "
end if
sql = sql&" AND ((Stockholder.stockholder_group)='"&n&"')) AND ( VOTE.STATUS = YES) and ((committee.committee_date = #"&s_date&"#)) order by  Stockholder.stockholder_name"
'response.write sql

  Set rs=Server.CreateObject("ADODB.recordset")
  rs.open sql,conn,1,3
   do until rs.eof or rs.bof 
    	response.write  "<li><a href=vote.asp?vote_id=" & rs(3) & "&question_id=" & request("question_id") & "&stockholderid="&rs(1)& "&sub_question_id=" & request("sub_question_id") & ">" & rs(0) & "</a><br>"
 	i = i+1
   rs.movenext
   loop
   rs.close
