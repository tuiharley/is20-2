...
<% 	if rs1.eof or rs1.bof then 
		 If rs(3) > time()  then%>
		<li><a target="right" href=stock_list.asp?question_id=<%=rs(0) %>&sub_question_id=0><font class=f1><%=rs(1) %></font></a>
		<% Else  %>
	...
	do until rs1.eof
		If rs(3) > time()  then%>
		<li><a target="right" href=stock_list.asp?question_id=<%=rs(0) %>&sub_question_id=<%= rs1(0) %>><font class=f1><%=rs1(1) 	%></font></a>			
		<% 
		else%>
