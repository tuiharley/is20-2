...
<table border="1" cellspacing="1" width="100%" id="AutoNumber1" bgcolor="#9900FF" height="26" bordercolor="#9900FF">
  <tr>
    ...
    <span lang="th">แก้ไขรายชื่อผู้มีสิทธิร่วมการลงคะแนนเสียง</span></font></b></td>
  </tr>
</table>
<p align="left">
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<form name="stockholder" action="edit_stocklist.asp"  method="post">
...
elseif sub_question_id = "0" then
  ...
  rs.open sql,conn,1,3
   ...
   %>
    	<input type="checkbox" name="C<%=i%>"  value="<%=rs(1)%>" <%if rs(3) then%>checked<%end if %> ><%= rs(0)%><br>
   <%
 			i = i+1
   rs.movenext
   loop
...
&nbsp;</p>
<a target="right" href=question_edit_index.asp><font class=f1><< ย้อนกลับ</font></a></form>
</body>
