    ...
    <tr> 
      <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr> 
            ...
            <td width="15%"><div align="center"><img name="ok" src="../images/true.gif" width="25" height="25" style="cursor:hand" onClick="top.HideWindow('DialogS1');"><img  name="close" src="../images/DELETE.ICO" width="25" height="25" style="cursor:hand" Onclick="if(confirm('ต้องการออกโดยไม่บันทึก')==true){top.RunWindow('Temp1','close_search_specialist.asp'); }"></div></td>
          </tr>
        </table></td>
    </tr>
    <tr> 
...
if empcode<>"" then
	Sql = Sql & " AND (EMPLOYEE.EMP_ID like '"&empcode&"%') AND (EMPLOYEE.EMP_ID NOT IN (SELECT EMP_ID FROM APP_EMPAPP_TMP WHERE USER_ID = " & Session("User_Id") & "))"
end if
if empname<>"" then
	Sql = Sql&"AND ((EMPLOYEE.NAME_THAI LIKE '%"&empname&"%') OR (EMPLOYEE.SURNAME_THAI LIKE '%"&empname&"%')) AND (EMPLOYEE.EMP_ID NOT IN (SELECT EMP_ID FROM APP_EMPAPP_TMP WHERE USER_ID = " & Session("User_Id") & "))"
end if
...
%>
          <tr> 
            <td><div align="center"><font size="2"><input type="button" name="send" value="<%=Rs("EMP_ID")%>" class="button" onMouseOver="this.className='buttonover'" onMouseOut="this.className='button'" onClick="top.RunWindow('Temp1','add_search_specialist.asp?empcode=<%=Rs("EMP_ID")%>&empcodetmp=' + document.form1.empcode.value);"></font></div></td>
            <td width="1" bgcolor="#3399FF"><img src="../images/blank.gif" width="1" height="1"></td>
...
top.ShowRunWindow('DialogS1','add_specialist.asp?empcode='+document.form1.empcode.value+'&empname='+document.form1.empname.value);
}
</script>
