          ...
          <td><table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
              ...
              <tr> 
                ...
                <td valign="top"><table border="3" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#FFFFFF" width="100%">
                    <tr> 
                      <td colspan="1" valign="top"><div align="center"><font color="#4B0097" size="3"><strong>การใช้บริการพิเศษ 
                          (SPC)</strong></font></div></td>
                    </tr>
                    <%
	SQL = "SELECT SPCID FROM SPC_CUST_USE WHERE (CG_CODE = '" & cgcode & "') ORDER BY SPCID"
	Rs2.Open SQL, Session("Conn1"),1,3	

	SQL = "SELECT SPCID, SPCNAME FROM SPC ORDER BY SPCID"
	Rs.Open SQL, Session("Conn1"),1,3	
	
	do while not Rs.EOF
%>
                    <tr> 
                      <td><input name="SPC" type="checkbox" id="SPC" value="<%=Rs("SPCID")%>" <%if not Rs2.EOF and not Rs2.BOF then%><%if Cint(Rs("SPCID"))=Cint(Rs2("SPCID")) then%><%Rs2.MoveNext%>checked<%end if%><%end if%>>
                        <font size="2" color="#4B0097"><%=Rs("SPCNAME")%></font></td>
                    </tr>
                    <%
		Rs.MoveNext
	loop
...
%>
        ...
        <tr> 
          <td><div align="center"> 
              <input name="send" type="button" id="send" value="บันทึกข้อมูล" class="button" onMouseOver="this.className='buttonover'" onMouseOut="this.className='button'" onClick="if(confirm('ยืนยันการบันทึกข้อมูล')==true){dosend();}">
              &nbsp;&nbsp; 
...
function dosend(){
	...
	top.RunWindow('Temp1','temp_save_custmain_add7.asp');
}
</script>
</body>
</html>