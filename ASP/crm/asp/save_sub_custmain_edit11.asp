...
<%
	...
	Sql = "DELETE MKT_CUSTOMER WHERE MC_CUSTOMER_ID = '"& cus_id &"' "
...
%>
<script language=javascript>
	top.<%=Request.QueryString("oldiframe")%>.document.body.style.cursor='wait'; 
	top.RunWindow('<%=Request.QueryString("oldiframe")%>','custmain_add11.asp?cgcode=<%=left(cgcode,5) &"01001"%>');
	top.HideWindow('<%=Request.QueryString("iframe")%>');
</script>
</BODY>
</HTML>
