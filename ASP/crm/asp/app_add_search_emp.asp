...
<%
	...
	Dim iframe, selectname, empcode, empname
	iframe = Request.QueryString("iframe")
	selectname = Request.QueryString("selectname")
...
if empcode<>"" then
	Sql = Sql&"AND (EMPLOYEE.EMP_ID like '"&empcode&"%') "
end if
if empname<>"" then
...
%>
          <tr> 
            <td><div align="center"><font size="2"><input type="button" name="send" value="<%=Rs("EMP_ID")%>" class="button" onMouseOver="this.className='buttonover'" onMouseOut="this.className='button'" onClick="top.<%=iframe%>.document.form1.<%=selectname%>.value='<%=Rs("EMP_ID")%>'; top.HideWindow(self.name);"></font></div></td>
            <td width="1" bgcolor="#3399FF"><img src="../images/blank.gif" width="1" height="1"></td>
...
function dosubmit(){
	...
	top.ShowRunWindow(self.name,'app_add_search_emp.asp?iframe=<%=iframe%>&selectname=<%=selectname%>&empcode='+document.form1.empcode.value+'&empname='+document.form1.empname.value);
}
</script>
</body>
</html>
