...
%>
<% 
	Dim i, selectdate, opendate1, opendate2, emp1, emp2, emp1_name, emp2_name
	...
	SQLE = "SELECT APPOINT_STATUS.AS_STATUS_ID, APPOINT_STATUS.AS_STATUS_NAME, APPOINT.APP_DATEAP, APPOINT.APP_DATECT, APPOINT_STATUS.AS_STATUS_NAME, APPOINT.APP_REMARK, APPOINT.APP_CUST_NAME, APPOINT.APP_TEL_CONTACT, APPOINT.APP_FAX_NO, APPOINT.APP_EMAIL, EMPLOYEE.NAME_THAI, EMPLOYEE.SURNAME_THAI, APPOINT.APP_EMPAPP1, APPOINT.APP_EMPAPP2 FROM APPOINT INNER JOIN APPOINT_STATUS ON APPOINT.AS_STATUS_ID = APPOINT_STATUS.AS_STATUS_ID INNER JOIN EMPLOYEE ON APPOINT.APP_EMPAPP = EMPLOYEE.EMP_ID WHERE (APPOINT.APP_ID = " & app_id & ")"
...
%>
	...
	for i=0 to 59
%>
                    <option value="<%=right("0"&i,2)%>" <%if right("0"&i,2)=right((FormatDateTime(RsE("APP_DATEAP"),4)),2) then%>selected<%end if%>><%=right("0"&i,2)%></option>
                    <%
	next
%>
              ...
              <tr> 
                ...
                <td colspan="2"><input name="showdate2" type="text" class="text" id="showdate2" size="18" readonly value="<%=left(FormatDateTime(RsE("APP_DATECT"),1),len(FormatDateTime(RsE("APP_DATECT"),1))-4)&right(FormatDateTime(RsE("APP_DATECT"),1),4)+543%>"> 
                  ...
                  <input name="popdate2" type="button" id="popdate2" value="....." class="button" onMouseOver="this.className='buttonover'" onMouseOut="this.className='button'" onClick="top.ShowRunPositionSizeWindow('Dialog1','280','120','250px','260px','calendar-sdmy-ddmy.asp?input1=top.Desktop3.document.form1.opendate2&input2=top.Desktop3.document.form1.showdate2');"> 
                  <font  size="2" color="#4B0097">&nbsp;&nbsp;เวลา</font> <select name="h2" id="h2" onkeypress="if(window.event.keyCode==13){document.form1.m2.focus();}">
                    <%
	for i=0 to 23
%>
                    <option value="<%=right("0"&i,2)%>" <%if right("0"&i,2)=left((FormatDateTime(RsE("APP_DATECT"),4)),2) then%>selected<%end if%>><%=right("0"&i,2)%></option>
                    <%
	next
%>
                  </select>
	...
	for i=0 to 59
%>
                    <option value="<%=right("0"&i,2)%>" <%if right("0"&i,2)=right((FormatDateTime(RsE("APP_DATECT"),4)),2) then%>selected<%end if%>><%=right("0"&i,2)%></option>
                    <%
	next
%>
                  </select> <font  size="2" color="#FF93C9">(hh.mm)</font></td>
              ...
              <tr> 
                ...
                <td><font  color="#4B0097" size="2"> 
                  <select size="1" name="app_status" onkeypress="if(window.event.keyCode==13){document.form1.APP_REMARK.focus();}">
					...
					%>
                    <option value="<%=RsS("AS_STATUS_ID")%>" <%if RsS("AS_STATUS_ID")=RsE("AS_STATUS_ID") then%>selected<%end if%>><%=RsS("AS_STATUS_NAME")%></option>
                    <%
				RsS.MoveNext
			    loop
                RsS.Close
	...
	Rs.Open SQL,Session("Conn1"),1,3
              ...
              <tr> 
                ...
                <td colspan="2"><input name="app_fax_no" type="text" class="text" onKeyPress="if(window.event.keyCode==13){document.form1.APP_MAIL.focus();}" value="<% =RsE("APP_FAX_NO") %>" size="20" maxlength="50"></td>
              </tr>
              <tr> 
			  ...
			  %>
                ...
                <td colspan="2"><input name="APP_EMP_TMP" type="button" class="button" id="APP_EMP_TMP" onMouseOver="this.className='buttonover'" onMouseOut="this.className='button'" value="เพิ่มรายชื่อ" onClick="top.ShowRunCenterSizeWindow('DialogS1','300px','650px','add_specialist2.asp?app_id=<%=app_id%>&cgcode=<%=cgcode%>')">
				<!--
...
%>
              ...
              <tr> 
                ...
                <td colspan="2"><textarea rows="3" name="APP_REMARK" cols="50"><%=RsE("APP_REMARK")%></textarea> 
                  ...
                  <input name="selectdate" type="hidden" id="selectdate" value="<%=selectdate%>"> 
                </td>
              </tr>
            </table></td>
        </tr>
        <tr> 
          <td><div align="center"> 
              ...
              &nbsp;&nbsp; 
              <input id="back" type="button" value=" กลับหน้าเดิม " name="back" class="button" onMouseOver="this.className='buttonover'" onMouseOut="this.className='button'" onClick="top.ShowRunWindow('Desktop2','appoint2.asp?selectdate=<%=selectdate%>');top.HideWindow('Desktop3');">
            </div></td>
        </tr>
        <tr> 
          <td>&nbsp;</td>
...
<Script language="JavaScript">
	...
	function dosend(){
		...
		if(checkdate(document.form1.opendate1.value,document.form1.opendate2.value)){
			alert('กรุณาระบุชันที่นัดหมายให้ถูกต้อง');
			return;
		}
		...
		if(confirm('ยืนยันการบันทึกข้อมูล')){
			...
			top.RunWindow('Temp1','temp_save_app_edit2.asp');
		}
	}
</Script>
</html>
