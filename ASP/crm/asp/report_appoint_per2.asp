...
<%
	Dim totalno, i
		 ...
		 '   Sql = " SELECT CUST_GROUP.CG_NAME, APPOINT.APP_EMPAPP, EMPLOYEE.TITLE_THAI, EMPLOYEE.NAME_THAI, EMPLOYEE.SURNAME_THAI, CUST_GROUP.CG_CODE, CG_ADDRESS.CGADDR_SNAME, CG_ADDRESS.CGADDR_BAND, CG_ADDRESS.CGADDR_SOI, CG_ADDRESS.CGADDR_STREET, CG_ADDRESS.CGADDR_TUMBOL,  CG_ADDRESS.CGADDR_DISTRICT, CG_ADDRESS.CGADDR_PROVINCE, CG_ADDRESS.CGADDR_POSTCODE, APPOINT.APP_ID, APPOINT.APP_DATECT FROM  CUST_GROUP INNER JOIN  APPOINT ON CUST_GROUP.CG_CODE = APPOINT.CG_CODE INNER JOIN CG_ADDRESS ON CUST_GROUP.CGADDR_ID = CG_ADDRESS.CGADDR_ID INNER JOIN  EMPLOYEE ON APPOINT.APP_EMPAPP = EMPLOYEE.EMP_ID WHERE (MONTH(APPOINT.APP_DATECT) = " & monthapp & ") AND (APPOINT.APP_EMPAPP = '" & emp_id & "')"
			Sql = "SELECT CUST_GROUP.CG_NAME, EMPLOYEE.TITLE_THAI, EMPLOYEE.NAME_THAI, EMPLOYEE.SURNAME_THAI, CUST_GROUP.CG_CODE, CG_ADDRESS.CGADDR_SNAME, CG_ADDRESS.CGADDR_BAND, CG_ADDRESS.CGADDR_SOI, CG_ADDRESS.CGADDR_STREET, CG_ADDRESS.CGADDR_TUMBOL, CG_ADDRESS.CGADDR_DISTRICT, CG_ADDRESS.CGADDR_PROVINCE, CG_ADDRESS.CGADDR_POSTCODE, CONTACT.CT_EMPMET, MONTH(CONTACT.CT_DATEMET) AS Month, CONTACT.CT_DATEMET,  CONTACT.CT_ID FROM  CUST_GROUP INNER JOIN CG_ADDRESS ON CUST_GROUP.CGADDR_ID = CG_ADDRESS.CGADDR_ID INNER JOIN CONTACT ON CUST_GROUP.CG_CODE = CONTACT.CG_CODE INNER JOIN  EMPLOYEE ON CONTACT.CT_EMPMET = EMPLOYEE.EMP_ID WHERE (CONTACT.CT_EMPMET = '" & emp_id & "') AND (MONTH(CONTACT.CT_DATEMET) = " & monthapp & ")"
...
%>			
            <tr style="cursor:hand"onClick="top.ShowRunWindow('Desktop4','con_detail.asp?con_id=<%=Rs("CT_ID")%>&cgcode=<%=Rs("CG_CODE")%>');">
              <td width="1" bgcolor="#3399FF"><img src="../images/blank.gif" width="1" height="1"></td>
			  ...
			  <%=day(Rs("CT_DATEMET")) & "/" & month(Rs("CT_DATEMET")) & "/" & year(Rs("CT_DATEMET"))+543%></font></div></td>
              <td width="1" bgcolor="#3399FF"><div align="center"><font color="#4B0097" size="2"><img src="../images/blank.gif" width="1" height="1"></font></div></td>
