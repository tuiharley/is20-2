...
<%
	...
	Dim cus_id, cbus, cmcode, cgcode, oldiframe
	cus_id = Request.QueryString("cus_id")
	cbus = Request.QueryString("cbus")
	cmcode = Request.QueryString("cmcode")
	cgcode = Request.QueryString("cgcode")
	oldiframe = Request.QueryString("iframe")

if cbus="" then
	Sql = "SELECT CG_CODE, MC_CUSTOMER_ID FROM MKT_CUSTOMER WHERE (MC_CUSTOMER_ID = '"&cus_id&"') AND (NOT (CG_CODE IN ('"&cgcode&"'))) "
	Rs.Open Sql,Session("Conn1"),1,3
		cbus = left(Rs("CG_CODE"),2)
		cmcode = left(Rs("CG_CODE"),5)
		cgcode = Rs("CG_CODE")
	Rs.Close
end if
      ...
      <td><table width="95%" border="3" align="center" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF" style="border-collapse: collapse">
          ...
          <tr> 
            ...
            <td  valign="bottom"><div align="left"> 
                <select name="cbus" id="cbus" onkeypress="if(window.event.keyCode==13){document.form1.cmcode.focus();}" onchange="document.body.style.cursor='wait'; top.RunWindow(self.name,'sub_custmain_edit11.asp?cus_id=<%=cus_id%>&cbus='+this.value);">
...
%>
          ...
          <tr> 
            ...
            <td valign="bottom"><div align="left"> 
                <select name="cmcode" id="cmcode" onkeypress="if(window.event.keyCode==13){document.form1.cgcode.focus();}" onchange="document.body.style.cursor='wait'; top.RunWindow(self.name,'sub_custmain_edit11.asp?cus_id=<%=cus_id%>&cbus='+document.form1.cbus.value+'&cmcode='+this.value);">
...
%>
                &nbsp; &nbsp; 
                <input name="search1" type="button" id="search1" value="ค้นหา" class="button" onMouseOver="this.className='buttonover'" onMouseOut="this.className='button'" onClick="top.ShowRunCenterSizeWindow('DialogSS1','300px','450px','sub_custmain_edit11_search_cmcode.asp?iframe='+self.name+'&cus_id=<%=cus_id%>&cbus=<%=cbus%>')">
                &nbsp; &nbsp;
                <input name="add1" type="button" id="add1" class="button" onMouseOver="this.className='buttonover'" onMouseOut="this.className='button'" onClick="top.Dialog2.document.body.style.cursor='wait'; top.ShowRunCenterSizeWindow('Dialog2','300px','550px','sub_custmain_edit11_maincomp_add.asp?cus_id=<%=cus_id%>&iframe='+self.name);" value=" เพิ่ม ">
                &nbsp;&nbsp; 
                <input name="edit1" type="button" id="edit1" class="button" onMouseOver="this.className='buttonover'" onMouseOut="this.className='button'" onClick="top.Dialog2.document.body.style.cursor='wait'; top.ShowRunCenterSizeWindow('Dialog2','300px','550px','sub_custmain_edit11_maincomp_edit.asp?cus_id=<%=cus_id%>&cbus=<%=cbus%>&cmcode=<%=cmcode%>&iframe='+self.name);" value="แก้ไข">
                <%
	end if
%>
              </div></td>
...
%>
                &nbsp;&nbsp; 
                <input name="search2" type="button" id="search2" value="ค้นหา" class="button" onMouseOver="this.className='buttonover'" onMouseOut="this.className='button'" onClick="top.ShowRunCenterSizeWindow('DialogSS1','300px','450px','sub_custmain_edit11_search_cgcode.asp?iframe='+self.name+'&cus_id=<%=cus_id%>&cbus=<%=cbus%>&cmcode=<%=cmcode%>&prov=<%=Rs("province_id")%>')">
                &nbsp;&nbsp; 
                <input name="add2" type="button" id="add2" class="button" onMouseOver="this.className='buttonover'" onMouseOut="this.className='button'" onClick="top.Dialog2.document.body.style.cursor='wait'; top.ShowRunCenterSizeWindow('Dialog2','450px','600px','sub_custmain_edit11_maingroup_add.asp?cus_id=<%=cus_id%>&cbus=<%=cbus%>&cmcode=<%=cmcode%>&area=<%=Rs("area")%>&prov=<%=Rs("PROVINCE_ID")%>&iframe='+self.name);" value=" เพิ่ม ">
                &nbsp;&nbsp; 
                <input name="edit2" type="button" id="edit2" class="button" onMouseOver="this.className='buttonover'" onMouseOut="this.className='button'" onClick="top.Dialog2.document.body.style.cursor='wait'; top.ShowRunCenterSizeWindow('Dialog2','450px','600px','sub_custmain_edit11_maingroup_edit.asp?cus_id=<%=cus_id%>&cbus=<%=cbus%>&cmcode=<%=cmcode%>&area=<%=Rs("area")%>&prov=<%=Rs("PROVINCE_ID")%>&cgcode='+document.form1.cgcode.value+'&iframe='+self.name);" value="แก้ไข">
                <%
	end if
%>
              </div></td>
...
<%end if%>
	...
	function dosend(){
		...
		if(confirm('ยืนยันการบันทึกข้อมูล')){
			top.RunWindow('Temp1','save_sub_custmain_edit11.asp?cus_id=<%=cus_id%>&cmcode=<%=cmcode%>&cgcode='+document.form1.cgcode.value+'&ccat='+document.form1.ccat.value+'&cbus=<%=cbus%>&oldiframe=<%=oldiframe%>&iframe='+self.name);
		}
	}
</script>
</body>
