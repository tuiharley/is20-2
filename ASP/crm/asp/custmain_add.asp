...
<body bgcolor="#E8F8FF" onLoad="document.form1.bus_website2.focus();">
<%
...
'Response.End()
                ...
                <td valign="top"><table border="3" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#FFFFFF" width="100%">
                    ...
                    <tr> 
                      <td width="30%"><div align="right"><font size="2" color="#4B0097">Website 
                          http://</font></div></td>
                      <td width="68%"><input name="bus_website2" class="text" size="50" maxlength="50" value="<%if not Rs.EOF then Response.Write(Rs("WEB"))end if%>"></td>
                    </tr>
                    <tr> 
                      <td height="22"><div align="right"><font size="2" color="#4B0097">หมายเลขโทรศัพท์</font></div></td>
                      <td><font size="1" color="#4B0097"> 
                        <input name="bus_phone2" class="text" size="20" maxlength="20" value="<%if not Rs.EOF then Response.Write(Rs("TEL"))end if%>">
                        </font></td>
                    </tr>
                    <tr> 
                      <td><div align="right"><font size="2" color="#4B0097">Fax</font></div></td>
                      <td><font size="1" color="#4B0097"> 
                        <input name="bus_fax2" class="text" size="20" maxlength="20" value="<%if not Rs.EOF then Response.Write(Rs("FAX"))end if%>">
                        </font></td>
                    </tr>
                    <tr> 
                      <td><div align="right"><font size="2" color="#4B0097">จำนวนพนักงานในสำนักงาน</font></div></td>
                      <td><font color="#4B0097" size="1"> 
                        <input name="bus_companyemp2" class="text"  size="10" maxlength="10" value="<%if not Rs.EOF then Response.Write(Rs("NUM_EMP1"))end if%>">
                        &nbsp; คน</font></td>
                    </tr>
                    <tr> 
                      <td><div align="right"><font size="2" color="#4B0097">จำนวนพนักงานในโรงงาน</font></div></td>
                      <td><font color="#4B0097" size="1"> 
                        <input name="bus_branchemp2" class="text" size="10" maxlength="10" value="<%if not Rs.EOF then Response.Write(Rs("NUM_EMP2"))end if%>">
                        &nbsp; คน</font></td>
                    </tr>
                    <tr> 
                      ...
                      <td> <select size="1" name="select_type">
                          ...
                          <option value="1" <%if not Rs.EOF then%><%if not isnull(Rs("RELATION")) then%><%if Cint(Rs("RELATION")) = Cint(1) then  %>selected<%end if%><%end if%><%end if%>>ต้องปรับปรุง</option>
                          <option value="2" <%if not Rs.EOF then%><%if not isnull(Rs("RELATION")) then%><%if Cint(Rs("RELATION")) = Cint(2) then %>selected<%end if %><%end if%><%end if%>>พอใช้</option>
                          <option value="3" <%if not Rs.EOF then%><%if not isnull(Rs("RELATION")) then%><%if Cint(Rs("RELATION")) = Cint(3) then %>selected<%end if %><%end if%><%end if%>>ดี</option>
                          <option value="4" <%if not Rs.EOF then%><%if not isnull(Rs("RELATION")) then%><%if Cint(Rs("RELATION")) = Cint(4) then %>selected<%end if %><%end if%><%end if%>>ดีมาก</option>
                        </select></td>
                    </tr>
                    <tr> 
                      <td valign="top"><div align="right"><font color="#4B0097" size="2">เพิ่มเติม</font></div></td>
                      <td><textarea rows="5" name="text_area" cols="60" onKeyPress="if(document.form1.APP_REMARK.value.length>=500){event.returnValue=false;}"><% if not Rs.EOF then Response.Write(Rs("DETAIL"))end if%></textarea></td>
                    </tr>
                  </table></td>
              </tr>
            </table></td>
   ...
   <input name="text_area" type="hidden" id="text_area">
	...
	function dosend(){
		...
		alert('กรุณาระบุจำนวนพนักงานในสำนักงานให้ถูกต้อง');
			document.form1.bus_companyemp2.focus();
		return;
		}
		...
		alert('กรุณาระบุจำนวนพนักงานในโรงงานให้ถูกต้อง');
		document.form1.bus_branchemp2.focus();
		return;
		}
