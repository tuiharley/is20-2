...
<%
	...
	Rs.Open SQL,Session("Conn1"),1,3
		body2 = body2 & "<br><dd>ท่านมีนัดหมายเข้าพบลูกค้า รหัส " & cgcode & " ชื่อ " & Rs("CM_NAME") & " " & Rs("CG_NAME") & " ในวันที่ "& opendate2 &"</dd> "
	Rs.Close
	...
	Sql = "SELECT EMP_ID FROM APP_EMPAPP_TMP WHERE USER_ID = '" & Session("User_Id") & "' "
	Rs.Open Sql,Session("Conn1"),1,3
	if Rs.Recordcount <> 0 then
			do while not Rs.EOF 
				Sql = "INSERT INTO APP_EMPAPP  VALUES ( "  & app_id & ", '" & cgcode & "' ,  '"  & Rs("EMP_ID") & "')" 
				Rs2.Open Sql,Session("Conn1"),1,3
				Rs.Movenext
			loop
			Rs.Close
			Sql = "SELECT EMP_ID FROM APP_EMPAPP WHERE (APP_ID = " & app_id & ") AND (CG_CODE = '" & cgcode & "') "
			Rs.Open Sql,Session("Conn1"),1,3
			
				do while not Rs.EOF 
					body = ""
					Sql = "SELECT EMP_ID, TITLE_THAI, NAME_THAI, SURNAME_THAI, EMAIL FROM EMPLOYEE WHERE EMP_ID='" & Rs("EMP_ID") & "' "
					Rs2.Open Sql,Session("Conn1"),1,3
						if Rs2("EMAIL") <> ""  then 
							emailto = Rs2("EMAIL")
