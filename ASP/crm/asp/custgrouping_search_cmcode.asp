...
%>
          <tr> 
            <td><div align="center"><font size="2"><input type="button" name="send" value="<%=Rs("CM_CODE")%>" class="button" onMouseOver="this.className='buttonover'" onMouseOut="this.className='button'" onClick="top.ShowRunWindow('<%=iframe%>','custgrouping.asp?cus_id=<%=cus_id%>&cbus=<%=cbus%>&cmcode=<%=Rs("CM_CODE")%>'); top.HideWindow(self.name);"></font></div></td>
            <td width="1" bgcolor="#3399FF"><img src="../images/blank.gif" width="1" height="1"></td>
...
function dosubmit(){
	...
	top.ShowRunWindow(self.name,'custgrouping_search_cmcode.asp?iframe=<%=iframe%>&cus_id=<%=cus_id%>&cbus=<%=cbus%>&cmname='+document.form1.cmname.value);
}
</script>
</body>
</html>
